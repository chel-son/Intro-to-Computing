...
function updateCommentCount() {
    ...
    const comments = JSON.parse(localStorage.getItem('negroscomments')) || [];
...
function addComment() {
    ...
    if (backgroundInput.files && backgroundInput.files[0] && userLocation && commentText) {
        ...
        reader.onload = function (e) {
            ...
            let comments = JSON.parse(localStorage.getItem('negroscomments')) || [];
            comments.push(newComment);
            localStorage.setItem('negroscomments', JSON.stringify(comments));
