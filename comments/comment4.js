...
function addComment() {
    ...
    if (backgroundInput.files && backgroundInput.files[0] && userLocation && commentText) {
        ...
        reader.onload = function (e) {
            ...
            let comments = JSON.parse(localStorage.getItem('comment4')) || [];
            comments.push(newComment);
            localStorage.setItem('comment4', JSON.stringify(comments));
...
function editComment(timestamp) {
    const comments = JSON.parse(localStorage.getItem('comment4')) || [];
...
window.addEventListener('load', function() {
    const threadAdd = JSON.parse(localStorage.getItem('threadAdd4')) || [];
