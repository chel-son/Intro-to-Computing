...
function addComment() {
    ...
    if (backgroundInput.files && backgroundInput.files[0] && userLocation && commentText) {
        ...
        reader.onload = function (e) {
            ...
            let comments = JSON.parse(localStorage.getItem('comment3')) || [];
            comments.push(newComment);
            localStorage.setItem('comment3', JSON.stringify(comments));
...
function editComment(timestamp) {
    const comments = JSON.parse(localStorage.getItem('comment3')) || [];
...
window.addEventListener('load', function() {
    const threadAdd = JSON.parse(localStorage.getItem('threadAdd3')) || [];
