...
function updateCommentCount() {
    ...
    const comments = JSON.parse(localStorage.getItem('palawancomments')) || [];
...
function addComment() {
    ...
    if (backgroundInput.files && backgroundInput.files[0] && userLocation && commentText) {
        ...
        reader.onload = function (e) {
            ...
            let comments = JSON.parse(localStorage.getItem('palawancomments')) || [];
            comments.push(newComment);
            localStorage.setItem('palawancomments', JSON.stringify(comments));
