...
function addComment() {
    ...
    if (backgroundInput.files && backgroundInput.files[0] && userLocation && commentText) {
        ...
        reader.onload = function (e) {
            ...
            let comments = JSON.parse(localStorage.getItem('comment5')) || [];
            comments.push(newComment);
            localStorage.setItem('comment5', JSON.stringify(comments));
...
function editComment(timestamp) {
    const comments = JSON.parse(localStorage.getItem('comment5')) || [];
...
window.addEventListener('load', function() {
    const threadAdd = JSON.parse(localStorage.getItem('threadAdd5')) || [];
